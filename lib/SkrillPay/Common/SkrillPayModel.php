...
use SkrillPay\Validation\JsonValidator;
...
class SkrillPayModel
{
    ...
    private function _convertToArray($param)
    {
        ...
        foreach ($param as $k => $v) {
            if ($v instanceof SkrillPayModel) {
                $ret[$k] = $v->toArray();
        ...
        if (sizeof($ret) <= 0) {
            $ret = new SkrillPayModel();
        }


        foreach ($ret as $rk=>$rt){
           if(is_array($rt)){
               foreach ($rt as $rtk=>$r){
                   $_ret[$rtk] = $r;
               }
    ...
    /**
     * Fills object value from html string
     * @param $string
     * @return $this
     */
    public function fromHtml($string){

        preg_match('/<meta\s+http-equiv=refresh\s+content="0; url=([\w\W]*?)"/si', $string, $matches);
        return  $this->fromArray(['approval_url'=>$matches[1]]);
     ...
     * @return $this
     */
    public function fromString($sid){

        $sid = $sid ? SkrillPayConstants::OPENID_REDIRECT_LIVE_URL . "?sid=".$sid : "";
        return  $this->fromArray(['approval_url'=>$sid]);
